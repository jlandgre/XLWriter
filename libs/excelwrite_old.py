...
def XLWriterPrep(lst_dfs, lst_shts, lst_fmts, lst_colwidths, df,sht):
    ...
    for i in range(len(df.columns)+1): # +1 accounts for index but should modify for multi-index possibility
        lst_fmts[len(lst_fmts) - 1].append('')
        lst_colwidths[len(lst_colwidths) - 1].append(10)
    return lst_dfs, lst_shts, lst_fmts, lst_colwidths

# Write one or more DataFrames to Excel workbook (JDL 10/24/19)
...
# Returns the Excel letter combination corresponding to an integer column
# number, icol (base 1)
...
def CreateExcelStepsDF(lst_dfs, lst_XLshts, ColInfo):
    ...
    for dframe, sht in zip(lst_dfs, lst_XLshts):
        ...
        for i, col in enumerate(lst_dfCols):
            ...
            comment = []
            if not isinstance(ColInfo[DescCI][col] , float) and np.isnan(ColInfo[UnitsCI][col]):
                comment = ColInfo[DescCI][col]

            if isinstance(not ColInfo[UnitsCI][col], float) and np.isnan(ColInfo[UnitsCI][col]):
            #if (not ColInfo[UnitsCI][col] is np.nan):
                comment = comment + ' in ' + ColInfo[UnitsCI][col]
        ...
        #Add a blank row to create spacing in the recipe
...
def WriteExcelWorkbook(lst_dfs, lst_shts, fName_xlsx, ColInfo, IsExcelSteps):
    ...
    print('back from CreateExcelStepsDF')
    i=0
